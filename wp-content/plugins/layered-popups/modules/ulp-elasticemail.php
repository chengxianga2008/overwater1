...
class ulp_elasticemail_class {
	...
	function popup_options_check($_errors) {
		...
		if (isset($_POST["ulp_elasticemail_enable"])) $popup_options['elasticemail_enable'] = "on";
	...
	function popup_options_populate($_popup_options) {
		...
		if ($ulp->ext_options['enable_customfields'] == 'on') {
			$fields = array();
			foreach($this->fields as $key => $value) {
				if (isset($_POST['ulp_elasticemail_field_'.$key])) {
					$fields[$key] = stripslashes(trim($_POST['ulp_elasticemail_field_'.$key]));
				}
			}
