			...
			<table class="ulp_useroptions">
				...
				<tr>
					...
					<td>
						...
						<br /><em>'.__('Enter Facebook App ID. <a href="admin.php?page=ulp-faq#ulp-facebook-app">How to obtain App ID.</a> If your theme or another plugin already use Facebook App ID, you can leave this field blank to avoid conflicts.', 'ulp').'</em>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						...
						<br /><em>'.__('Enter Google Client ID. <a href="admin.php?page=ulp-faq#ulp-google-app">How to obtain Client ID.</a> If your theme or another plugin already use Facebook App ID, you can leave this field blank to avoid conflicts.', 'ulp').'</em>
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<br /><em>'.__('Enter Google API Key. <a href="admin.php?page=ulp-faq#ulp-google-app">How to obtain API Key.</a> If your theme or another plugin already use Google API Key, you can leave this field blank to avoid conflicts.', 'ulp').'</em>
					</td>
				</tr>
			</table>';
...
<script>(function(d, s, id) {
  ...
  fjs.parentNode.insertBefore(js, fjs);
	...
	function front_popup_content($_content, $_popup_options) {
		global $ulp;
		$popup_options = array_merge($this->default_popup_options, $_popup_options);
		$facebook_button = '<a class="ulp-submit-button ulp-submit-facebook'.($popup_options['button_inherit_size'] == 'on' ? ' ulp-inherited' : '').'" onclick="return ulp_subscribe_facebook(this);"'.($ulp->options['fa_enable'] == 'on' ? ' data-icon="fa-facebook"' : '').' data-label="'.esc_html($popup_options['social2_facebook_label']).'" data-loading="'.esc_html($popup_options['button_label_loading']).'">'.($ulp->options['fa_enable'] == 'on' ? '<i class="fa fa-facebook"></i>&nbsp; ' : '').esc_html($popup_options['social2_facebook_label']).'</a>';
		$google_button = '<a class="ulp-submit-button ulp-submit-google'.($popup_options['button_inherit_size'] == 'on' ? ' ulp-inherited' : '').'" onclick="return ulp_subscribe_google(this);"'.($ulp->options['fa_enable'] == 'on' ? ' data-icon="fa-google"' : '').' data-label="'.esc_html($popup_options['social2_google_label']).'" data-loading="'.esc_html($popup_options['button_label_loading']).'">'.($ulp->options['fa_enable'] == 'on' ? '<i class="fa fa-google"></i>&nbsp; ' : '').esc_html($popup_options['social2_google_label']).'</a>';
